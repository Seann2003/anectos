...
// Correct vote type structure for SPL Governance SDK
const SINGLE_CHOICE_VOTE_TYPE = {
  choiceType: "single",
  // Alternative structures you might need to try:
  // choiceType: { single: {} },
  // or: { type: "singleChoice" },
  // or: { singleChoice: {} }
};
  ...
  const createProposal = async () => {
    ...
    try {
      ...
      const ixCtx = await SPL_GOVERNANCE.createProposalInstruction(
        name.trim(),
        description.trim(),
        SINGLE_CHOICE_VOTE_TYPE, // voteType with correct structure
        [optionLabel.trim() || "Activate"], // options as array
        false, // useDenyOption
        REALM_PK,
        GOVERNANCE_PK,
        ...
        walletPk, // payer - keep using wallet as payer
        proposalSeed
      );
      ...
      if (ixCtx && Array.isArray(ixCtx.instructions)) {
        ixCtx.instructions.forEach((instruction: any) => tx.add(instruction));
      } else if (ixCtx && ixCtx.ix) {
        tx.add(ixCtx.ix);
      } else if (ixCtx) {
        // If ixCtx is directly an instruction
        tx.add(ixCtx);
      } else {
        throw new Error(
      ...
      // Sign with the random keypair first
      tx.partialSign(randomKeypair);

      // Then use Privy to get the user's signature
  ...
  return (
    <div className="max-w-xl mx-auto p-6 space-y-6">
      ...
      <p className="text-sm text-gray-600">
        This page creates a proposal using existing Realm / Governance /
        TokenOwnerRecord. Replace placeholder public keys in the file with real
        ones before using on-chain.
      </p>
      ...
      <div className="text-[10px] text-gray-500 space-y-1 border-t pt-4">
        <p>
          Note: This only creates the base proposal account. Adding signatories,
          inserting transactions, and starting voting require additional
          instructions (addSignatoryInstruction, insertTransactionInstruction,
          etc.).
        </p>
        <p>
          Active funding stage context: the proposal text/option can encode
          moving a project to 'active'; executing that would require a custom
          instruction in a transaction inserted later.
        </p>
        <p className="text-red-600 font-medium">
          Debug: If you still get the choiceType error, check your
          SPL_GOVERNANCE SDK documentation for the exact vote type structure it
          expects.
        </p>
      </div>
    </div>
