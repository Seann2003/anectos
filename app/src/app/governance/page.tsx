...
export default function GovernancePage() {
  ...
  useEffect(() => {
    // Load projects from API (falls back to local approved items if present)
    let mounted = true;
    async function load() {
      ...
      try {
        ...
        // If the API returns { items: [...] } use that, otherwise accept an array
        ...
        const mapped: Project[] = (items || []).map((it: any) => ({
          id:
            ...
            it.id ??
            it.project_id ??
            String(it.projectPubkey ?? Math.random()),
          title: it.project?.title ?? it.title ?? "Untitled project",
          shortDescription:
            it.projectMeta?.short_description ??
            it.project?.shortDescription ??
            it.shortDescription ??
            "",
          requestedAmountUsd: Number(
            it.project?.requested_amount_usd ??
              it.project?.requestedAmountUsd ??
              it.requestedAmountUsd ??
              0
          ),
          proposer: it.project?.owner ?? it.proposer ?? "unknown",
          imageUrl: it.projectMeta?.image_uri ?? it.imageUrl,
          quorum: Number(it.project?.quorum ?? 60),
          yesVotes: Number(it.project?.yes_votes ?? it.yesVotes ?? 0),
          noVotes: Number(it.project?.no_votes ?? it.noVotes ?? 0),
        }));

        if (!mounted) return;
        // merge with any approved items in localStorage (keeps previous behaviour)
        try {
          const raw =
            typeof window === "undefined"
              ? null
              : localStorage.getItem("governance.approved.v1");
          let approvedMapped: Project[] = [];
          if (raw) {
            const approved = JSON.parse(raw) as Array<any>;
            approvedMapped = approved.map((a) => ({
              id: `approved-${a.id}`,
              title: a.title,
          ...
          const combined = [...approvedMapped, ...mapped];
          setProjects(combined);
          if (combined.length && !selectedId) setSelectedId(combined[0].id);
        } catch (e) {
          setProjects(mapped);
        }
      } catch (err: any) {
        ...
        // fallback to mock projects so UI remains usable
    ...
    try {
      ...
      transaction.add(
        SystemProgram.transfer({
          ...
          lamports: 100, // Minimal amount (0.0000001 SOL)
        })
      );
      ...
      toast.success("Vote submitted", {
        description: `You voted ${voteChoice?.toUpperCase()} with ${voteWeight.toLocaleString()} ACTS on "${
          selected.title
        }". Transaction: ${receipt.signature.slice(0, 8)}...`,
      });
    } catch (err) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 mt-2">
        Governance is disabled in DB-only mode. Review projects awaiting
        whitelisting and cast your token-weighted vote.
      </p>
